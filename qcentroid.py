...
SPECIALIST_ID = "MIP"
SPECIALIST_SOURCE = (
    "github.com/georgekorpas/opt-specialists/blob/main/03_MIP_specialist.md"
)
...
def solver(input_data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        return _error_result(phase="adapter", exc=exc, started_utc=started_utc, t0=t0, dataset_sha256=dataset_sha256)
    ...
    try:
        ...
        return _error_result(phase="solver", exc=exc, started_utc=started_utc, t0=t0, dataset_sha256=dataset_sha256)
    ...
    avg_machine_utilization_pct = _utilization_pct(schedule, internal["machines"], horizon)
...
def run(data: Dict[str, Any], solver_params: Dict[str, Any] = None, extra_arguments: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    kwargs = dict(
        max_exec_time_m=float(extra_arguments.get("max_exec_time_m", solver_params.get("max_exec_time_m", 10.0))),
...
def _on_time_pct(schedule, jobs):
    ...
    on_time = sum(1 for j in jobs if by_job.get(j["id"]) is not None and by_job[j["id"]] <= j.get("due_date", 10**9))
...
def _gantt_data(schedule, machines):
    return [
        {"task_id": f'{s["job_id"]}.{s.get("op_id", 0)}', "resource_id": s["machine_id"],
